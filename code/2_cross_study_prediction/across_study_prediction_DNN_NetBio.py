...
## Run DNN
output_dir = '../../result/2_cross_study_prediction/DNN_NetBio'
for train_idx, train_dataset in enumerate(train_datasets):
	for test_idx, test_dataset in enumerate(studies):
		...
		common_pathways = list(set(train_epdf['pathway'].tolist()) & set(test_epdf['pathway'].tolist()))
		...
		epdf1, epdf2 = train_epdf, test_epdf
		train_epdf = expression_StandardScaler(train_epdf)
		test_epdf = expression_StandardScaler(test_epdf)
		

		## NetBio pathways
		NetBio_pathways = return_proximal_pathways(train_edf, target, nGene, adj_pval)
		train_epdf = train_epdf.loc[train_epdf['pathway'].isin(NetBio_pathways),:]
		test_epdf = test_epdf.loc[test_epdf['pathway'].isin(NetBio_pathways),:]


		### train test expression data
		X_train, X_test = train_epdf.T.values[1:].astype(np.float), train_epdf.T.values[1:].astype(np.float)
